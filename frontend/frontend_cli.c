...
ret_e main_opt_body(CLI_IGNORE_CTX, char *rel_pathname) {
    if (did_run_main_opt) {
    ...
    if (!util_does_path_exist(rel_pathname)) {
        FRONTEND_PRINT(FRONTEND_ERR, FRONTEND_PRINT_PREFIX,
                       "path '%s' doesn't exist on disk.", rel_pathname);

        return RET_FAIL;
    }

    if (!util_is_path_a_file(rel_pathname)) {
        FRONTEND_PRINT(FRONTEND_ERR, FRONTEND_PRINT_PREFIX,
                       "path '%s' isn't a file.", rel_pathname);

        return RET_FAIL;
    }

    if (!util_validate_file_ext(rel_pathname, STARLANG_FILE_EXT)) {
    ...
    char *filename = strrchr(rel_pathname, '/') + 1; // +1 skips the slash

    char parent_path[256];
    realpath(rel_pathname, parent_path);
    parent_path[strlen(parent_path) - strlen(filename)] = '\0';

    if (frontend_entrypoint(rel_pathname, parent_path, filename))
        return RET_NORMAL;

    FRONTEND_PRINT(FRONTEND_ERR, FRONTEND_PRINT_PREFIX,
                   "failed to run '%s'. exiting.", rel_pathname);
